...
function ytArgs(platform) {
    ...
    // --no-warnings          : keep stderr clean for parseProgress
    // --no-check-certificates: bypass SSL issues on some hosts
    // --socket-timeout 15    : don't hang on unreachable URLs
    // NOTE: --retry-sleep "exponential=1::2" is INVALID yt-dlp syntax → was crashing yt-dlp
    // Valid syntax per yt-dlp docs: exp=START[:END[:BASE]]  e.g. "exp=1:8:2"
    // Removed entirely for maximum compatibility — retries alone are sufficient
    const a = [
        "--no-playlist",
        "--no-warnings",
    ...
    if (isYT(platform)) {
        a.push("--extractor-retries", "3");
        // "--retry-sleep", "exp=1:8:2"  ← valid syntax, uncomment if needed
    }
    return a;
}
...
/* ─────────────────────────────────────────────────
   SITEMAPS & ROBOTS — BEFORE static middleware.
   Express serves routes in registration order.
   If a sitemap.xml file exists on disk, express.static
   would return it before this route is ever reached.
   Fix: register these routes first, THEN static.
   ⚠️  Also DELETE any sitemap.xml / robots.txt files
   from your project folder — they are no longer needed.
───────────────────────────────────────────────── */

/* Sitemap index → tells crawlers about both sitemaps */
...
/* Core pages: /, /how-to-use, /faq × all 20 languages */
...
/* SEO pages: 8 platforms × 20 languages = 160 URLs */
...
/* robots.txt — blocks API endpoints, advertises all three sitemaps */
...
/* ================= STATIC FILES ================= */
/* Registered AFTER sitemap/robots so dynamic routes win over disk files */
app.use(express.static(__dir, { index: false }));

/* ─────────────────────────────────────────────────
   SEO router — after static, before /:lang/* catchall.
   The router's internal SLUG_INDEX guard ensures it
   never intercepts /fr/, /fr/faq, /fr/download etc.
───────────────────────────────────────────────── */
...
/* ================= API: fetch-info (FIXED) ================= */
app.get("/fetch-info", (req, res) => {
    const { url } = req.query;
    if (!url) return res.status(400).json({ error:"No URL" });
    const p = getPlatform(url);

    // FIX: use --dump-single-json (more reliable than --dump-json for playlists/shorts)
    // FIX: increase timeout to 35s — YouTube info fetch can be slow on first hit
    // FIX: collect stderr separately so we can log real errors without polluting stdout parse
    const args = [
        ...ytArgs(p),
        "--dump-single-json",
        "--flat-playlist",        // never recurse into playlists
    ];

    // Platform-specific extraction args
    if (p === "tiktok") {
        args.push("--extractor-args", "tiktok:api_hostname=api16-normal-c-useast1a.tiktokv.com");
    }
    if (p === "instagram") {
        // Instagram needs a broader format selection for metadata
        args.push("--extractor-args", "instagram:api_hostname=i.instagram.com");
    }

    args.push(url);

    const proc = spawn("yt-dlp", args);
    let out = "", errOut = "", timer;
    ...
    // FIX: 35s timeout — YouTube bot-check can add a few seconds
    timer = setTimeout(() => {
        try { proc.kill("SIGKILL"); } catch {}
        if (!res.headersSent) res.status(504).json({ error:"Timeout — try again" });
    }, 35000);
    ...
    proc.on("close", exitCode => {
        ...
        // FIX: don't bail on non-zero exit if we still got JSON output
        // yt-dlp sometimes exits 1 but still prints valid JSON for age-gated videos
        const raw = out.trim();
        if (!raw) {
            const msg = errOut.includes("Sign in") ? "Login required for this video"
                      : errOut.includes("Private") ? "This video is private"
                      : errOut.includes("unavailable") ? "Video unavailable"
        ...
        let info;
        try {
            // Take the last complete JSON line (some platforms print warnings before JSON)
            const lines = raw.split("\n").filter(l => l.trim().startsWith("{"));
            info = JSON.parse(lines[lines.length - 1] || lines[0]);
        } catch (e) {
            console.error("❌ fetch-info JSON parse error:", e.message);
            return res.status(500).json({ error:"Parse error — invalid response" });
        }

        // ── Thumbnail resolution ──────────────────────────────────────────────
        // FIX Instagram: pick highest-res thumbnail that isn't an .ico
        // FIX: prefer thumbnails array over single thumbnail field (more reliable)
        let thumb = null;
        if (info.thumbnails && info.thumbnails.length > 0) {
            // Sort by resolution descending, fallback to array order
            const sorted = [...info.thumbnails]
                .filter(t => t.url && !t.url.includes(".ico"))
                .sort((a, b) => ((b.width||0)*(b.height||0)) - ((a.width||0)*(a.height||0)));
            thumb = sorted[0]?.url || info.thumbnails[info.thumbnails.length - 1]?.url || null;
        }
        if (!thumb) thumb = info.thumbnail || null;

        // FIX Instagram: ALL thumbnails must go through proxy (Instagram CDN blocks direct browser requests)
        if (p === "instagram" && thumb) {
            thumb = "/thumb-proxy?url=" + encodeURIComponent(thumb);
        }
        // FIX TikTok: thumbnails also need proxy for CORS
        if (p === "tiktok" && thumb) {
            thumb = "/thumb-proxy?url=" + encodeURIComponent(thumb);
        }

        // ── Duration ─────────────────────────────────────────────────────────
        const dur = info.duration_string
            || (info.duration
        ...
        res.json({
            ...
            platform:  p,
            has1080:   !NO1080.includes(p),
            isMusic:   p === "youtube_music",
        });
    });
    ...
    proc.on("error", err => {
        ...
        if (!res.headersSent) res.status(500).json({ error:"yt-dlp not found or crashed" });
    });
});

/* ================= API: thumb-proxy (FIXED) ================= */
app.get("/thumb-proxy", (req, res) => {
    const { url } = req.query;
    if (!url) return res.status(400).send("No URL");

    // FIX: decode the URL first (it may have been double-encoded)
    let targetUrl;
    try { targetUrl = decodeURIComponent(url); } catch { targetUrl = url; }

    // FIX: validate it's actually a URL before proxying (security)
    ...
    // FIX: proper headers per CDN — Instagram, TikTok & YouTube all need different Referers
    const isIg = targetUrl.includes("cdninstagram") || targetUrl.includes("instagram");
    const isTt = targetUrl.includes("tiktok") || targetUrl.includes("tiktokcdn");
    const referer = isIg ? "https://www.instagram.com/"
                  : isTt ? "https://www.tiktok.com/"
    ...
    const request = lib.get(targetUrl, options, (r) => {
        // FIX: follow redirects (Instagram CDN often 301/302)
        if (r.statusCode === 301 || r.statusCode === 302) {
            const location = r.headers["location"];
            if (location) {
                r.resume(); // drain the old response
                // Recursive redirect follow (max 1 hop)
                const rLib = location.startsWith("https") ? https : http;
                rLib.get(location, options, (r2) => {
                    res.setHeader("Content-Type", r2.headers["content-type"] || "image/jpeg");
                    res.setHeader("Cache-Control", "public, max-age=86400");
                    res.setHeader("Access-Control-Allow-Origin", "*");
                    r2.pipe(res);
                }).on("error", () => { if (!res.headersSent) res.status(502).send("Redirect proxy error"); });
                return;
            }
        }

        if (r.statusCode && r.statusCode >= 400) {
            console.error("thumb-proxy HTTP", r.statusCode, "for", targetUrl.slice(0,80));
            r.resume();
            return res.status(502).send("CDN returned " + r.statusCode);
        }
    ...
    // FIX: timeout so a slow CDN doesn't hang the connection forever
    request.setTimeout(10000, () => {
        request.destroy();
        if (!res.headersSent) res.status(504).send("Thumbnail fetch timeout");
    });
});

/* ================= API: get-size (FIXED) ================= */
app.get("/get-size", (req, res) => {
    const { url, type="video", quality } = req.query;
    if (!url) return res.status(400).json({ error:"No URL" });
    ...
    // FIX: add --quiet so only the --print output reaches stdout (no extra lines)
    const args = [
        ...ytArgs(p),
        "--quiet",
        "-f", fmt,
        "--print", "%(filesize,filesize_approx,NA)s",
    ];
    if (p === "tiktok") {
        args.push("--extractor-args", "tiktok:api_hostname=api16-normal-c-useast1a.tiktokv.com");
    }
    ...
    timer = setTimeout(() => {
        try { proc.kill(); } catch {}
        if (!res.headersSent) res.json({ size: null, bytes: null });
    }, 20000);
    ...
    proc.on("close", () => {
        ...
        if (!raw || raw === "NA") return res.json({ size: null, bytes: null });
        const b = parseInt(raw);
        if (!b || isNaN(b)) return res.json({ size: null, bytes: null });
        let s;
        if      (b >= 1073741824) s = (b / 1073741824).toFixed(2) + " GB";
        else if (b >= 1048576)    s = (b / 1048576).toFixed(1)    + " MB";
        else if (b >= 1024)       s = (b / 1024).toFixed(0)        + " KB";
        else                      s = b + " B";
...
app.get("/download-file", async (req, res) => {
    try {
        ...
        // FIX: for MP3 always use bestaudio; for video use platform-specific format
        const fmt = type === "mp3" ? "bestaudio/best" : videoFormat(p, quality);

        // ── Step 1: Get title + approximate size ─────────────────────────────
        // FIX: use separate --print calls with --quiet so stdout is ONLY the printed value
        // FIX: the original single spawn mixed title|||size which breaks if title has "|||"
        // FIX: use --no-simulate so size estimation is skipped for platforms that block it
        const metadata = await new Promise(resolve => {
            ...
            // Platform-specific args for metadata pass
            if (p === "tiktok") {
                metaArgs.push("--extractor-args", "tiktok:api_hostname=api16-normal-c-useast1a.tiktokv.com");
            }
            metaArgs.push(url);

            const pr = spawn("yt-dlp", metaArgs);
            let o = "", e = "";
            pr.stdout.on("data", d => { o += d.toString(); });
            pr.stderr.on("data", d => { e += d.toString(); });

            const timer = setTimeout(() => {
                try { pr.kill(); } catch {}
                resolve({ title: "video", bytes: null });
            }, 20000);

            pr.on("close", () => {
                clearTimeout(timer);
                const raw   = o.trim().split("\n")[0] || "";
                const parts = raw.split("|||");
                const rawTitle = (parts[0] || "").trim();
                const rawBytes = (parts[1] || "").trim();
                resolve({
                    title: rawTitle ? safeTitle(rawTitle) : "video",
                    bytes: rawBytes && rawBytes !== "NA" ? parseInt(rawBytes) || null : null,
                });
            });
        ...
        console.log(`⬇ [${type.toUpperCase()}] platform=${p} quality=${quality} title="${title}" size=${totalSize || "unknown"}`);

        // ── Step 2: Init progress tracking ───────────────────────────────────
        progressMap.set(id, { percent: 0, done: false });
        downloadProgress.set(id, { totalSize, bytesSent: 0, speed: "", eta: "" });

        // FIX: track bytes written to res using res.write interception, not res.on("data")
        // res.on("data") doesn't fire on ServerResponse — this was the cause of 0% progress
        let bytesReceived = 0;
        ...
        function trackChunk(chunk) {
            ...
            if (timeDiff >= 0.5) {
                const speed = (bytesReceived - lastBytes) / timeDiff;
        ...
        res.on("close", () => {
            progressMap.set(id, { percent: 100, done: true });
        });

        // FIX: safe filename — strip characters that break Content-Disposition
        ...
        // ─────────────────────────────────────────────────────────────────────
        if (type === "mp3") {
        // ── MP3 PATH (temp-file approach) ────────────────────────────────────
        //
        // WHY temp file instead of pipe→res:
        //   1. Chrome cancels downloads that don't send Content-Length up front.
        //   2. ffmpeg -i pipe:0 is unreliable on Windows for small/slow streams.
        //   3. Writing to a temp file first lets us know the exact size,
        //      set Content-Length, and send the complete file — no race condition.
        //
        // FLOW:
        //   yt-dlp  →  stdout pipe  →  ffmpeg  →  TEMP FILE
        //   then:   fs.stat(tmpFile)  →  res.sendFile()  →  fs.unlink()

            const br = ["320","256","192","128","96"].includes(String(quality)) ? String(quality) : "192";

            // Build unique temp file path (OS temp dir, never in project folder)
            const tmpDir  = require("os").tmpdir();
            const tmpName = `aio_mp3_${id}_${Date.now()}.mp3`;
            const tmpPath = path.join(tmpDir, tmpName);

            console.log(`[MP3] Starting — br=${br}k  tmp=${tmpPath}`);

            // yt-dlp: download best audio → stdout
            const ytArgs2 = [
                ...ytArgs(p),
                "-f", "bestaudio/best",
                "-o", "-",
            ...
            if (p === "tiktok") {
                ytArgs2.push("--extractor-args", "tiktok:api_hostname=api16-normal-c-useast1a.tiktokv.com");
            }
            ytArgs2.push(url);

            const yt = spawn("yt-dlp", ytArgs2, { stdio: ["ignore", "pipe", "pipe"] });

            // Log all yt-dlp stderr (always — not just errors)
            yt.stderr.on("data", d => {
                const line = d.toString().trim();
                if (line) console.log(`[yt-dlp mp3] ${line}`);
            });

            // ffmpeg: stdin pipe → encode → WRITE TO TEMP FILE (not stdout pipe)
            // Log full ffmpeg path + command for debugging
            console.log(`[ffmpeg] binary: ${FFMPEG}`);
            console.log(`[ffmpeg] exists: ${require("fs").existsSync(FFMPEG)}`);
            const ffArgs = [
                "-y",               // ← MUST be first: overwrite output without prompt
                "-loglevel",        "warning",
                "-probesize",       "50M",
                "-analyzeduration", "10M",
                "-i",               "pipe:0",    // read audio from yt-dlp stdout
                "-vn",                           // no video
                "-acodec",          "libmp3lame",
                "-b:a",             `${br}k`,
                "-ar",              "44100",
                "-ac",              "2",
                "-f",               "mp3",
                tmpPath,            // ← output file LAST (after -f mp3)
            ];
            console.log(`[ffmpeg] command: ${FFMPEG} ${ffArgs.join(" ")}`);
            const ff = spawn(FFMPEG, ffArgs, { stdio: ["pipe", "pipe", "pipe"] });

            // Log ALL ffmpeg stderr — this is the key diagnostic
            ff.stderr.on("data", d => {
                const line = d.toString().trim();
                if (line) console.log(`[ffmpeg mp3] ${line}`);
            });

            // Wire yt-dlp stdout → ffmpeg stdin
            yt.stdout.pipe(ff.stdin);

            // When yt-dlp finishes, close ffmpeg stdin so ffmpeg knows to flush+finish
            yt.on("close", (code) => {
                console.log(`[yt-dlp mp3] exited ${code}`);
                try { ff.stdin.end(); } catch {}
            });

            // Suppress EPIPE (client disconnected mid-download)
            yt.stdout.on("error", () => { try { ff.stdin.destroy(); } catch {} });
            ff.stdin.on("error", () => {});

            yt.on("error", (err) => {
                console.error("❌ yt-dlp spawn error:", err.message);
                if (!res.headersSent) res.status(500).send("yt-dlp not found");
            });

            ff.on("error", (err) => {
                console.error("❌ ffmpeg spawn error:", err.message);
                if (!res.headersSent) res.status(500).send("ffmpeg not found — check FFMPEG path in server.js");
            });

            // When ffmpeg finishes writing the temp file
            ff.on("close", (code) => {
                console.log(`[ffmpeg mp3] exited ${code}  tmp=${tmpPath}`);

                if (code !== 0) {
                    console.error(`❌ ffmpeg failed (exit ${code}) — MP3 not created`);
                    try { fs.unlinkSync(tmpPath); } catch {}
                    if (!res.headersSent) res.status(500).send("MP3 conversion failed");
                    progressMap.set(id, { percent: 100, done: true });
                    return;
                }
                ...
                try {
                    ...
                    console.error("❌ Temp file missing after ffmpeg:", statErr.message);
                    if (!res.headersSent) res.status(500).send("MP3 file not found after conversion");
                    progressMap.set(id, { percent: 100, done: true });
                    return;
                }

                if (fileSize < 1000) {
                    console.error(`❌ Temp file too small (${fileSize} bytes) — conversion failed silently`);
                    try { fs.unlinkSync(tmpPath); } catch {}
                    if (!res.headersSent) res.status(500).send("MP3 conversion produced empty file");
                    progressMap.set(id, { percent: 100, done: true });
                    return;
                }

                console.log(`[MP3] ✅ File ready: ${(fileSize/1024).toFixed(0)} KB — sending via res.download()`);

                progressMap.set(id, { percent: 99, done: false });

                // res.download() handles:
                //   - Content-Type: application/octet-stream (overridden below)
                //   - Content-Disposition: attachment; filename="..."
                //   - Content-Length from fs.stat internally
                //   - cleanup callback fires ONLY after the response is fully flushed
                //
                // We set Content-Type manually before calling download()
                // because res.download() defaults to octet-stream.
                res.setHeader("Content-Type", "audio/mpeg");

                res.download(
                    tmpPath,                    // absolute path to temp file
                    `${safeAscii}.mp3`,         // filename shown in browser Save dialog
                    { headers: {
                        "Content-Type":        "audio/mpeg",
                        "Content-Disposition": `attachment; filename="${safeAscii}.mp3"; filename*=UTF-8''${encodedName}.mp3`,
                        "Content-Length":      String(fileSize),
                    }},
                    (err) => {
                        // This callback fires AFTER the file is fully sent (or on error)
                        // Safe to delete the temp file here
                        if (err) {
                            // err.code === 'ECONNABORTED' means client cancelled — normal
                            if (err.code !== "ECONNABORTED" && err.code !== "EPIPE") {
                                console.error("❌ res.download error:", err.message);
                            } else {
                                console.log(`[MP3] Client disconnected — cleaning up ${tmpPath}`);
                            }
                        } else {
                            console.log(`[MP3] ✅ Download complete — cleaning up ${tmpPath}`);
                        }

                        // Delete temp file now that transfer is done
                        try { fs.unlinkSync(tmpPath); } catch {}
                        progressMap.set(id, { percent: 100, done: true });
                    }
                );
        ...
        // ── VIDEO PATH ───────────────────────────────────────────────────────
            ...
            res.setHeader("Content-Disposition",
                `attachment; filename="${safeAscii}.mp4"; filename*=UTF-8''${encodedName}.mp4`);

            // FIX: Instagram — yt-dlp sometimes outputs to stderr during video download
            // FIX: add --no-part so no temp files on disk
            const dlArgs = [
                ...ytArgs(p),
                "-f", fmt,
            ...
            // FIX: Instagram specific — suppress metadata write errors
            if (p === "instagram") {
                dlArgs.push("--no-write-info-json", "--no-embed-metadata");
            }
            // FIX: TikTok specific
            if (p === "tiktok") {
                dlArgs.push("--extractor-args", "tiktok:api_hostname=api16-normal-c-useast1a.tiktokv.com");
            }

            dlArgs.push(url);

            const pr = spawn("yt-dlp", dlArgs, { stdio: ["ignore", "pipe", "pipe"] });

            pr.stderr.on("data", d => {
                const line = d.toString();
                process.stdout.write(line);
                parseProgress(line, id);
            });

            // FIX: track chunks for progress instead of res.on("data")
            ...
            pr.on("close", (code) => {
                progressMap.set(id, { percent: 100, done: true });
                if (code !== 0) console.error(`[yt-dlp video] exited ${code}`);
            });

            pr.on("error", (err) => {
                console.error("❌ yt-dlp video spawn error:", err.message);
                if (!res.headersSent) res.status(500).send("Download failed");
            });

            // FIX: handle client disconnect — kill yt-dlp cleanly
            res.on("close", () => {
                try { pr.kill("SIGTERM"); } catch {}
            });
        }

    } catch (e) {
        console.error("❌ download-file exception:", e);
        if (!res.headersSent) res.status(500).send("Download failed");
    }
});
