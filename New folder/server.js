...
function ytArgs(platform) {
    // --no-playlist  : never download whole playlist by accident
    // --no-warnings  : clean stderr so parseProgress isn't polluted
    // --no-check-certificates : bypass SSL issues on some hosts
    // --socket-timeout 15     : don't hang forever on bad URLs
    // YouTube-specific:
    //   Do NOT add --js-runtime node — it breaks on most yt-dlp versions
    //   Add extractor-retries to recover from transient YouTube bot-check
    const a = [
        "--no-playlist",
        "--no-warnings",
    ...
    try {
        ...
        // ── MP3 PATH ─────────────────────────────────────────────────────────
            ...
            res.setHeader("Content-Disposition",
                `attachment; filename="${safeAscii}.mp3"; filename*=UTF-8''${encodedName}.mp3`);

            const br = ["320","256","192","128","96"].includes(String(quality)) ? String(quality) : "192";

            // FIX: build yt-dlp args for audio pipe
            const ytArgs2 = [
                ...ytArgs(p),
                "-f", "bestaudio/best",
                "-o", "-",              // output to stdout
            ...
            // FIX: parse progress from stderr but don't crash if yt-dlp writes non-progress lines
            yt.stderr.on("data", d => {
                const line = d.toString();
                parseProgress(line, id);
                // Also log real errors
                if (line.includes("ERROR") || line.includes("error")) {
                    console.error("[yt-dlp mp3 err]", line.trim());
                }
            });

            // FIX: ffmpeg args — add -probesize and -analyzeduration so ffmpeg
            // doesn't timeout waiting for enough data before starting encode
            // FIX: add -loglevel error so ffmpeg stderr is quiet in production
            ...
            // FIX: handle ffmpeg stderr
            ff.stderr.on("data", d => {
                const line = d.toString().trim();
                if (line) console.error("[ffmpeg]", line);
            });

            // FIX: handle yt-dlp exit before ffmpeg finishes — close ffmpeg stdin
            yt.on("close", (code) => {
                try { ff.stdin.end(); } catch {}
                if (code !== 0) console.error(`[yt-dlp] exited ${code} during MP3 download`);
            });

            // FIX: handle yt-dlp write errors (e.g. client disconnected mid-download)
            yt.stdout.on("error", () => { try { ff.stdin.destroy(); } catch {} });
            ff.stdin.on("error",  () => {}); // suppress EPIPE when client disconnects

            // Pipe: yt-dlp audio → ffmpeg → response
            yt.stdout.pipe(ff.stdin);
            ff.stdout.on("data", chunk => {
                trackChunk(chunk);
                // Don't use pipe here so we can track chunks
            ...
            yt.on("error", (err) => {
                console.error("❌ yt-dlp MP3 spawn error:", err.message);
